// ═══════════════════════════════════════════════════════════════════════════
// EL HANDLER DEL WEBHOOK DE CAL.COM, EN `lib` (ARQ-B1, auditoría 28).
//
// Antes vivía en `src/app/api/webhook/calcom/route.ts` y `calcom.ts` lo
// invocaba con un `import()` dinámico + un `Request` fabricado a mano — una
// dependencia invertida (lib → app → lib) invisible al `grep` estático por
// ser dinámica. `route.ts` seguía importando `calcomConfig`/
// `secretoCalcomConfigurado`/`verificarFirmaCalcom` de aquí (app → lib), así
// que el ciclo completo cruzaba la frontera dos veces.
//
// Ahora el handler ES código de `lib`: `route.ts` solo lo re-exporta como
// `POST`, y `calcom.ts` lo importa ESTÁTICAMENTE para la entrega local. La
// entrega local sigue pasando por ESTA MISMA función — no por un atajo que
// repita la validación a mano — precisamente para que un endurecimiento
// futuro del webhook público (otro esquema de firma, un campo más exigido)
// se aplique a los dos caminos a la vez, sin que la entrega local se entere
// tarde de que dejó de cumplir el contrato.
// ═══════════════════════════════════════════════════════════════════════════
...
import { calcomConfig, secretoCalcomConfigurado, verificarFirmaCalcom } from './calcom';
...
/** El handler real del webhook de Cal.com — `route.ts` lo re-exporta como
 * `POST` y `calcom.ts` lo llama para la entrega local (mismo Request+HMAC,
 * misma validación, mismo resultado). Ver la cabecera del archivo. */
export async function procesarWebhookCalcom(req: Request): Promise<Response> {
...
async function encontrarProspecto(email: string | null): Promise<VinculoProspecto> {
  ...
  // orden-no-importa: limit(2) sólo distingue cero, una o varias coincidencias; se rechaza si hay más de una.
