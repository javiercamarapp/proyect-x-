...
Un intervalo mayor a 90 segundos sin muestra detiene el proceso con motivo `telemetry-gap`; también se comprueba el tramo desde inicio hasta primera muestra y desde última muestra hasta fin, aunque pgbench ya haya terminado. Un retroceso del reloj se rechaza como `telemetry-clock-invalid`. El resumen exige muestras y continuidad en todos esos tramos, conserva los percentiles originales y usa la duración/tasa registradas al inicio. No basta con cumplir volumen y latencia si hubo un hueco. El runtime congelado de cualquier nueva corrida debe incluir `continuity.py` junto a `run.py` y los SQL. Esta mejora no modifica el runtime ni los resultados conservados del primer intento.

Regresión sin base, red ni esperas reales: `python3 scripts/ci/innovativos-capacidad/continuity_test.py`. Simula un día completo y demuestra que huecos internos, iniciales o finales invalidan una corrida aunque pasen duración, volumen y percentiles.
