...
export async function prepareBuild(target, env = process.env, deps = {}) {
  ...
  try {
    ...
    stage = 'PULLED_ENV';
    if (!lstatSync(file).isFile() || lstatSync(file).isSymbolicLink()) throw new Error('FILE'); // eslint-disable-line security/detect-non-literal-fs-filename -- target allowlisted, archivo propio CI.
    const original = readFileSync(file, 'utf8'); // eslint-disable-line security/detect-non-literal-fs-filename -- target allowlisted.
    ...
    stage = 'WRITE';
    try {
      chmodSync(file, 0o600); // eslint-disable-line security/detect-non-literal-fs-filename -- archivo verificado, propio del runner.
      writeFileSync(file, hydrated, { mode: 0o600 }); // eslint-disable-line security/detect-non-literal-fs-filename -- archivo verificado, propio del runner.
      stage = 'BUILD';
      ...
      stage = 'RESTORE';
      writeFileSync(file, original, { mode: 0o600 }); // eslint-disable-line security/detect-non-literal-fs-filename -- restaura sólo el archivo propio tras éxito/fallo.
      stage = previousStage;
    }
