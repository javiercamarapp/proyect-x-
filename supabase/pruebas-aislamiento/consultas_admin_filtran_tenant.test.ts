...
const ALLOWLIST: Record<string, string> = {
  ...
  'src/lib/admin/calcom_webhook.ts': '`prospecto` es el CRM global de LIKIDA y tiene `tenant_id` NULL durante adquisición (0105). El handler del webhook firmado de Cal.com debe encontrar por correo al lead capturado antes del calendario; exigir un tenant impediría enlazar precisamente todos los leads pre-cierre. La escritura queda anclada al id devuelto por esa búsqueda y las tablas son deny-all. (ARQ-B1, auditoría 28: este handler vivía en `src/app/api/webhook/calcom/route.ts` — la razón de la exención no cambió, solo el archivo; `route.ts` ahora solo re-exporta `POST`.)',
